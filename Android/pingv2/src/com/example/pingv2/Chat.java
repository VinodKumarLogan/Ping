...
public class Chat extends Activity {
	...
	Button backButton,sendButton,refresh,clear;
	...
	List<Messages> dispMsg = new ArrayList<Messages>();
				...
				while(true)
				{
					try {
						...
						if(temp[0].equals("msg"))
						{
							...
							String receivedMessage = name+" : "+sentence.substring((temp[0].length()+1+temp[1].length()+1)); 
							msgs.add(receivedMessage);
							db.addMessage(new Messages(receivedMessage));
							runOnUiThread(new Runnable() { 
								public void run() {
									dispMsg = db.getAllMessages();
									String temp="";
									for(Messages m: dispMsg)
										temp+=m.getMessage()+"\n";										
